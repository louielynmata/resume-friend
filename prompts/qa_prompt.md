...
11. Use `CATEGORY: Category Name | comma-separated values` for compact skill and toolkit groups. Separate values with a comma followed by one space, never a semicolon. Do not replace this builder marker with a loose heading, bullet, Markdown table, unlabelled body line, or a legacy `Label: values` line without the `CATEGORY:` marker.
